...
bool Hwc3562::SupportPlatform(uint32_t soc_id){
  switch(soc_id){
    case 0x3562:
      return true;
    default:
...
struct assign_plane_group_3562{
	int display_type;
...
int Hwc3562::assignPlaneByHWC(DrmDevice* drm){
  ...
  for (auto &conn : drm->connectors()) {
    ...
    for(int i = 0; i < ARRAY_SIZE(assign_mask_default_3562);i++){
      if(display_id == assign_mask_default_3562[i].display_type){
        plane_mask = assign_mask_default_3562[i].drm_type_mask;
        break;
      }
    }
    ...
    if(plane_mask == 0){
      for(int i = 0; i < ARRAY_SIZE(assign_mask_default_3562);i++){
        if(assign_mask_default_3562[i].have_assigin == false){
          assign_mask_default_3562[i].display_type = display_id;
          plane_mask = assign_mask_default_3562[i].drm_type_mask;
          assign_mask_default_3562[i].have_assigin = true;
          break;
        }
      }
    }
...
int Hwc3562::assignPlaneByPlaneMask(DrmDevice* drm){
  std::vector<PlaneGroup*> all_plane_group = drm->GetPlaneGroups();
  for (auto &conn : drm->connectors()) {
    int display_id = conn->display();
    if(conn->state() != DRM_MODE_CONNECTED){
...
int Hwc3562::TryAssignPlane(DrmDevice* drm){
  int ret = -1;
