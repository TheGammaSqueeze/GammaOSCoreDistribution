...
int hdmi_edid_parse_speaker_allocation(unsigned char *buf, struct hdmi_audio_infors *infor)
{
  ...
  //  int length = buf[0] & 0x1f;
    ...
    ALOGV("%s : %d,buf[1]= 0x%x,buf[2]= 0x%x, layout = 0x%x",__FUNCTION__,__LINE__,buf[1],buf[2],infor->channel_layout);
    return 0;
}
    ...
    while (cur_offset < ddc_offset) {
        ...
        switch (tag) {
            case 0x02:  /* Video Data Block */
                ALOGV("%s: [CEA] Video Data Block",__FUNCTION__);
                break;
            case 0x01:  /* Audio Data Block */
                ALOGV("%s [CEA] Audio Data Block",__FUNCTION__);
                hdmi_edid_parse_cea_audio(buf + cur_offset,infor);
                break;
            case 0x04:  /* Speaker Allocation Data Block */
                ALOGV("%s: [CEA] Speaker Allocatio Data Block",__FUNCTION__);
                hdmi_edid_parse_speaker_allocation(buf + cur_offset,infor);
                break;
            case 0x03:  /* Vendor Specific Data Block */
                ALOGV("%s: [CEA] Vendor Specific Data Block",__FUNCTION__);
                break;
            case 0x05:  /* VESA DTC Data Block */
                ALOGV("%s: [CEA] VESA DTC Data Block",__FUNCTION__);
                break;
            case 0x07:  /* Use Extended Tag */
                ALOGV("%s: [CEA] Use Extended Tag Data Block %02x",__FUNCTION__,buf[cur_offset + 1]);
                switch (buf[cur_offset + 1]) {
                case 0x00:
                    ALOGV("%s: [CEA] Video Capability Data Block",__FUNCTION__);
                    break;
                case 0x05:
                    ALOGV("%s: [CEA] Colorimetry Data Block",__FUNCTION__);
                    break;
                case 0x06:
                    ALOGV("%s: [CEA] HDR Static Metedata data Block",__FUNCTION__);
                    break;
                case 0x0e:
                    ALOGV("%s: [CEA] YCBCR 4:2:0 Video Data Block",__FUNCTION__);
                    break;
                case 0x0f:
                    ALOGV("%s: [CEA] YCBCR 4:2:0 Capability Map Data",__FUNCTION__);
                    break;
            }
            break;
            default:
                ALOGV("%s: [CEA] unkowned data block tag",__FUNCTION__);
                break;
        }
...
int hdmi_edid_parse_extensions(unsigned char *buf, struct hdmi_audio_infors *infor)
{
    ...
    switch (buf[0]) {
        case 0xF0:
            ALOGV("%s: [EDID-EXTEND] Iextensions block map",__FUNCTION__);
            break;
        case 0x02:
            ALOGV("%s: [EDID-EXTEND] CEA 861 Series Extension",__FUNCTION__);
            hdmi_edid_parse_extensions_cea(buf, infor);
            break;
        case 0x10:
            ALOGV("%s: [EDID-EXTEND] Video Timing Block Extension",__FUNCTION__);
            break;
        case 0x40:
            ALOGV("%s: [EDID-EXTEND] Display Information Extension",__FUNCTION__);
            break;
        case 0x50:
            ALOGV("%s: [EDID-EXTEND] Localized String Extension",__FUNCTION__);
            break;
        case 0x60:
            ALOGV("%s: [EDID-EXTEND] Digital Packet Video Link Extension",__FUNCTION__);
            break;
        default:
            ALOGV("%s: [EDID-EXTEND] Unkowned Extension,tag = 0x%x",__FUNCTION__,buf[0]);
            return -1;
    }
...
int parse_hdmi_audio(struct hdmi_audio_infors *infor, int index)
{
    ...
    ALOGD("%s: %d: extendblock = %d",__FUNCTION__,__LINE__,extendblock);
    // paser extend block
    if(extendblock > 0) {
...
bool is_support_format(struct hdmi_audio_infors *infor, int hdmi_level) {
    ...
    for(i = 0; i < length; i++) {
        ...
        if(layout & HDMI_SPEAKER_ALLOCATION_TABLE[i].index) {
            ...
            if(offset+length < size){
                ...
                ALOGV("%s: buffer is not enought",__FUNCTION__);
                break;
            }
        }
    }

    ALOGV("%s: speaker allocation = %s",__FUNCTION__,buffer);
}
...
void dump(struct hdmi_audio_infors *infor)
{
    ...
    for(int i = 0; i < infor->number; i++) {
        ...
        ALOGV("%s: type = %s,channel = %d,sample = %s",__FUNCTION__,format,infor->audio[i].channel,sample);
    }
