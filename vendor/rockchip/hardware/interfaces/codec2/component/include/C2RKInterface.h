 ...
 */
template <typename T>
class C2RKInterface : public C2ComponentInterface {
public:
    C2RKInterface(const char *name, c2_node_id_t id, const std::shared_ptr<T> &impl)
        : mName(name),
          mId(id),
          mImpl(impl) {
    }

    ~C2RKInterface() override = default;
...
template<typename T>
using SimpleInterface = C2RKInterface<T>;
