...
traversalOrder = Parameter("traversalOrder", "TENSOR_INT32", "{4}", [0, 1, 3, 2])
blockMap = Parameter("blockMap", "TENSOR_INT32", "{2}", [0, 1])
dimFormat = Parameter("dimFormat", "TENSOR_INT32", "{4}", [0, 1, 0, 0])
dimensions = Parameter("dimensions", "TENSOR_INT32", "{4}", [2, 4, 2, 2])
d0ArrSegments = Parameter("d0ArrSegments", "TENSOR_INT32", "{0}", [])
d0ArrIndices = Parameter("d0ArrIndices", "TENSOR_INT32", "{0}", [])
d1ArrSegments = Parameter("d1ArrSegments", "TENSOR_INT32", "{3}", [0, 3, 4])
d1ArrIndices = Parameter("d1ArrIndices", "TENSOR_INT32", "{4}", [0, 1, 2, 1])
...
denseOut = Output("denseOut", "TENSOR_FLOAT32", "{4, 8}")
...
input0 = {sparseData: # input 0
          [1.0, 1.0, 0.0, 0.0, 2.0, 2.0, 3.0, 3.0, 0.0, 0.0, 4.0, 4.0, 5.0, 0.0, 0.0, 0.0]}

output0 = {denseOut: # output 0
           [1.0, 0.0, 2.0, 3.0, 0.0, 4.0, 0.0, 0.0, 1.0, 0.0, 2.0, 3.0, 0.0, 4.0, 0.0, 0.0,
...
quant8_asymm_signed = DataTypeConverter().Identify({
    sparseData: ("TENSOR_QUANT8_ASYMM_SIGNED", 2.5, -4),
    denseOut: ("TENSOR_QUANT8_ASYMM_SIGNED", 2.5, -4)
})

quant16_symm = DataTypeConverter().Identify({
    sparseData: ("TENSOR_QUANT16_SYMM", 3.125),
    denseOut: ("TENSOR_QUANT16_SYMM", 3.125)
})

quant16_asymm = DataTypeConverter().Identify({
    sparseData: ("TENSOR_QUANT16_ASYMM", 6.0, 1),
    denseOut: ("TENSOR_QUANT16_ASYMM", 6.0, 1)
})
